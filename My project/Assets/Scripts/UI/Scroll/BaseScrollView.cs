...
public class BaseScrollView : MonoBehaviour
{
    ...
    protected virtual BaseScrollItem RepositionItem(BaseScrollData data, int index, eScrollType type)
    {
        ...
        switch (scrollType)
        {
            ...
            case eScrollType.Bottom_To_Up:
            {
                posX = index % LineRow * ScrollItem.ItemSize.x;

                posY = scrollType == eScrollType.Top_To_Down
                    ? -(index / LineRow * ScrollItem.ItemSize.y)
                    ...
                    if (lastIndex < createCount)
                    {
                        for (int i = 0; i < count; i++)
                        {
                            ...
                            for (int line = 0; line < LineRow; line++)
                            {
                                ...
                                if (RectTransformUtility.RectangleContainsScreenPoint(ScrollRect.viewport, checkSize,
                                        Camera.main) == false)
                                {
